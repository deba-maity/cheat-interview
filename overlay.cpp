    ...
    switch (uMsg) {
        case WM_CREATE: {
            ...
            hStaticOutput = CreateWindowExA(
                ...
                10, 10, 660, 60, hwnd, (HMENU)101, GetModuleHandle(NULL), NULL);

            // Input (your question)
            hEditInput = CreateWindowExA(
                WS_EX_CLIENTEDGE, "EDIT", "",
                WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL,
                10, 80, 660, 25, hwnd, (HMENU)102, GetModuleHandle(NULL), NULL);
        ...
        case WM_KEYDOWN: {
            ...
            if (wParam == VK_RETURN && GetFocus() == hEditInput) {
                ...
                return 0; // swallow the Enter "ding"
            }
            break;
        }
...
Napi::Value ShowAnswer(const Napi::CallbackInfo& info) {
    ...
    if (hStaticOutput) SetWindowTextA(hStaticOutput, answer.c_str());
...
Napi::Value StartOverlay(const Napi::CallbackInfo& info) {
    ...
    // Run the overlay window in its own thread so Node stays responsive
    std::thread([](){
        ...
        // Make it semi-transparent (or set to 255 for opaque)
        SetLayeredWindowAttributes(hwnd, 0, 230, LWA_ALPHA);

        // ★ This is the key line: exclude from screen capture (Teams/Zoom/OBS)
